...
export const merge = createAlgorithmCard({
  id: 'merge',
...
private int[] Merge(int[] left, int[] right) {
  ...
  getInitialState: (p, t) => {
    ...
    return {
      ...
      stepPtr: 0,
      phase: 'READY',
      l: 0, r: n - 1, mid: Math.floor((n - 1) / 2),
      activeIndices: [],
      sortedIndices: [],
      comparisons: 0,
      log: {
    ...
    if (phase === 'READY' || phase === 'DONE') {
      ...
      if (currentStep.type === 'DIVIDE') {
        return {
          ...newState,
          phase: 'DIVIDING',
          l: currentStep.l, r: currentStep.r, mid: currentStep.mid,
          activeIndices: [currentStep.l, currentStep.r, currentStep.mid],
          log: { title: 'Dividing', type: 'info', messageKey: 'DIVIDING', params: { l: currentStep.l, r: currentStep.r, mid: currentStep.mid, midPlusOne: currentStep.mid + 1 } }
        };
      }
      ...
      if (currentStep.type === 'MERGE') {
        ...
        return {
          ...
          l: currentStep.l, r: currentStep.r, mid: currentStep.mid,
          leftArr, rightArr,
          i: 0, j: 0, k: currentStep.l,
          tempArr: [],
          activeIndices: [currentStep.l, currentStep.r],
          log: { title: 'Merging', type: 'info', messageKey: 'MERGING_SEGMENTS', params: { l: currentStep.l, mid: currentStep.mid, midPlusOne: currentStep.mid + 1, r: currentStep.r } }
        };
      }
    }
    ...
    if (phase === 'MERGING') {
      ...
      if (i < leftArr.length && j < rightArr.length) {
        ...
        if (leftArr[i] <= rightArr[j]) {
          const val = leftArr[i];
          return {
            ...newState, i: i + 1, tempArr: [...tempArr, val], activeIndices: [l + i, mid + 1 + j],
            log: { title: 'Taking Left', type: 'match', messageKey: 'TAKING_LEFT', params: { val } }
          };
        } else {
          const val = rightArr[j];
          return {
            ...newState, j: j + 1, tempArr: [...tempArr, val], activeIndices: [l + i, mid + 1 + j],
      ...
      if (i < leftArr.length) {
        const val = leftArr[i];
        return {
          ...newState, i: i + 1, tempArr: [...tempArr, val], activeIndices: [l + i],
      ...
      if (j < rightArr.length) {
        const val = rightArr[j];
        return {
          ...newState, j: j + 1, tempArr: [...tempArr, val], activeIndices: [mid + 1 + j],
      ...
      return {
        ...newState, array: newArray, phase: 'DONE', stepPtr: stepPtr + 1,
