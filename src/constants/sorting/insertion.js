...
export const insertion = createAlgorithmCard({
  id: 'insertion',
  ...
  getInitialState: (p, t) => {
    ...
    return {
      phase: 1, i: 1, j: 0, key: array[1],
      array, activeIndices: [1], sortedIndices: [0], swapIndices: [], comparisons: 0,
      log: {
  ...
  nextStep: (state) => {
    ...
    if (phase === 1) { // Selecting key
      ...
      return { 
        ...newState, phase: 2, key: array[i], j: i - 1, activeIndices: [i],
    ...
    if (phase === 2) { // Shifting
      if (j >= 0 && array[j] > key) {
        ...
        return { 
          ...newState, array: newArray, j: j - 1, activeIndices: [j, j + 1], swapIndices: [j, j + 1],
      ...
      const nextI = i + 1;
      return { 
        ...newState, array: newArray, phase: 1, i: nextI, 
        sortedIndices: [...new Array(nextI).keys()], activeIndices: [j + 1],
