...
export const cocktail = createAlgorithmCard({
  id: 'cocktail',
  ...
  algorithmPage: {
    ...
    extendedDescription: 'Cocktail Shaker Sort, also known as bidirectional bubble sort, improves upon bubble sort by traversing the list in both directions alternately.',
    legendItems: [
  ...
  getInitialState: (p, t) => {
    ...
    return {
      phase: 1, start: 0, end: array.length - 1, k: 0, swapped: false,
      direction: 1, passes: 1, array,
      activeIndices: [0, 1], sortedIndices: [], swapIndices: [], comparisons: 0,
      log: {
        title: 'PASS 1', type: 'info', messageKey: 'READY',
  ...
  nextStep: (state) => {
    ...
    if (phase === 1) { // Comparison
      ...
      if (isDone) {
        ...
        if (direction === 1) {
          ...
          return {
            ...newState, direction: -1, end: newEnd, k: nextK, swapped: false,
            passes: passes + 1, sortedIndices: [...sortedIndices, end],
            activeIndices: nextK >= start ? [nextK, nextK + 1] : [],
            log: { title: 'BACKWARD PASS', type: 'info', messageKey: 'BACKWARD_PASS', params: { end: end, k: nextK, start: start } }
          };
        } else {
          const newStart = start + 1;
          const nextK = newStart;
          return {
            ...newState, direction: 1, start: newStart, k: nextK, swapped: false,
            passes: passes + 1, sortedIndices: [...sortedIndices, start],
            activeIndices: nextK < end ? [nextK, nextK + 1] : [],
            log: { title: 'FORWARD PASS', type: 'info', messageKey: 'FORWARD_PASS', params: { start: start, k: nextK, end: end } }
          };
        }
      }
      ...
      if (array[k] > array[k + 1]) {
        return {
          ...newState, phase: 2,
          log: { title: 'OUT OF ORDER', type: 'mismatch', messageKey: 'OUT_OF_ORDER', params: { k: k, valK: array[k], kPlusOne: k + 1, valKPlusOne: array[k + 1] } }
        };
      }

      const nextK = k + direction;
      return {
        ...newState, k: nextK, activeIndices: (direction === 1 ? nextK < end : nextK >= start) ? [nextK, nextK + 1] : [],
        log: { title: direction === 1 ? 'FORWARD PASS' : 'BACKWARD PASS', type: 'match', messageKey: 'IN_ORDER', params: { k: k, valK: array[k], kPlusOne: k + 1, valKPlusOne: array[k + 1] } }
      };
    }
    ...
    if (phase === 2) { // Swap
      ...
      return {
        ...newState, array: newArray, phase: 1, swapped: true, 
        swapIndices: [k, k + 1], k: nextK, activeIndices: isKValid ? [nextK, nextK + 1] : [],
        log: { title: 'SWAPPED', type: 'shift', messageKey: 'SWAPPED', params: { k: k, kPlusOne: k + 1, valK: newArray[k], valKPlusOne: newArray[k + 1] } }
      };
    }
