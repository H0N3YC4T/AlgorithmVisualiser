...
const ArrayVisualizer = memo(({ array = [], activeIndices = [], sortedIndices = [], pivotIndex = -1, swapIndices = [] }) => {
  ...
  const activeSet = useMemo(() => new Set(Array.isArray(activeIndices) ? activeIndices : Array.from(activeIndices || [])), [activeIndices]);
  const sortedSet = useMemo(() => new Set(Array.isArray(sortedIndices) ? sortedIndices : Array.from(sortedIndices || [])), [sortedIndices]);
  const swappingSet = useMemo(() => new Set(Array.isArray(swapIndices) ? swapIndices : Array.from(swapIndices || [])), [swapIndices]);
  ...
  const bars = useMemo(() => {
    return itemsWithIds.map((item, i) => {
      ...
      if (isActive) colorClass = "bg-indigo-500 border-indigo-400 ring-4 ring-indigo-500/30 z-10 shadow-[0_0_15px_rgba(99,102,241,0.4)]";
      if (isPivot) colorClass = "bg-amber-400 border-amber-300 ring-4 ring-amber-400/40 z-20 shadow-[0_0_20px_rgba(251,191,36,0.4)]";
    ...
    return slotIds.map((slotId, i) => {
      ...
      return (
        <div key={slotId} className="flex-1 flex justify-center items-center">
          <span className={`text-[10px] font-black font-mono transition-colors ${isA || isP ? 'text-white' : 'text-slate-600'}`}>
            {i}
          </span>
        </div>
      );
