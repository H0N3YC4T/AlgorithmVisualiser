    ...
    <div className="overflow-x-auto pb-8 scrollbar-hide flex justify-center">
      <div className="inline-flex flex-col px-8 items-start">
        ...
        <div className="flex gap-1 mb-1">
          {target.split('').map((_, i) => ({ i, id: `idx-slot-${i}` })).map((item) => (
            <div key={item.id} className="w-10 flex-shrink-0 text-center text-[9px] font-black text-slate-700 font-mono">
              {item.i}
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-1">
          {target.split('').map((char, i) => ({ char, i, id: `tcell-slot-${i}` })).map((item) => {
            ...
            let cellClass = `${classCategory.cellBase} bg-slate-800 border-slate-700 text-slate-100`;
            
            if (isExplicitActive) {
              cellClass = `${classCategory.cellBase} bg-indigo-500/40 border-indigo-500 text-indigo-100 ring-4 ring-indigo-500/20 scale-105 z-10`;
            } else if (isLookAhead) {
              cellClass = `${classCategory.cellBase} bg-sky-400 border-sky-300 text-white shadow-xl shadow-sky-400/40 scale-110 z-10 animate-pulse`;
            } else if (isActiveChar) {
              cellClass = `${classCategory.cellBase} bg-blue-900/40 border-blue-500 text-blue-200 ring-4 ring-blue-500/20 scale-105`;
            } else if (isAccessed || isBehind) {
              cellClass = `${classCategory.cellBase} bg-slate-800/40 border-slate-800 text-slate-600`;
            }
        ...
        <div className="mt-4 flex gap-1 relative h-10">
          ...
          {pattern.split('').map((char, i) => ({ char, i, id: `pcell-slot-${i}` })).map((item) => {
            const isComparingIdx = phase === 1 && item.i === compIdx;
            const isMatched = (phase === 1 && !comparesRightToLeft && item.i < compIdx) || 
            ...
            let cellClass = `${classCategory.cellBase} bg-slate-700 border-slate-600 text-slate-100`;
            if (isComparingIdx) cellClass = `${classCategory.cellBase} bg-amber-400 border-amber-300 text-amber-950 ring-4 ring-amber-400/30 scale-110 z-20`;
            if (isMismatched) cellClass = `${classCategory.cellBase} bg-rose-500 border-rose-400 text-white`;
            if (isMatched) cellClass = `${classCategory.cellBase} bg-emerald-500 border-emerald-400 text-white`;
...
TextVisualizer.propTypes = {
  ...
  accessedIndices: PropTypes.oneOfType([
    PropTypes.instanceOf(Set),
    PropTypes.arrayOf(PropTypes.number)
  ]),
  activeIndices: PropTypes.oneOfType([
    PropTypes.instanceOf(Set),
    PropTypes.arrayOf(PropTypes.number)
  ]),
