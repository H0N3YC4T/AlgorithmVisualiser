...
const metricsBarRegistry = {
  category: `text-[18px] sm:text-[14px] md:text-[16px] font-black text-white/30 uppercase tracking-tighter`,
...
const localTheme = {
  ...
  metricPill:
    "hidden md:truncate lg:flex items-center gap-6 px-8 py-2.5 bg-slate-950/50 rounded-full border border-slate-800/50 shadow-inner",
...
export default function MetricsBar({
  ...
  return (
    <div className={`${localTheme.bar} px-3 md:px-6 gap-3 md:gap-6`}>
      <div className="flex items-center  gap-3 md:gap-6 overflow-hidden flex-1">
        ...
        <div className="flex-row items-center gap-2 overflow-hidden flex-1">
          <span className={`${localTheme.category} hidden md:inline flex-1 `}>{algorithm.category}</span>
          <h1 className={`${localTheme.name} truncate sm:flex-1`}>{name}</h1>
        </div>
      </div>
      ...
      <div className="flex flex-row sm:flex-col">
        ...
        <div className={localTheme.controlGroup}>
          <button
            onClick={reset}
            className={`${localTheme.btnBase} ${localTheme.btnReset} px-3 md:px-4`}
            title="Hard Reset"
          >
          ...
          <div className="w-px h-6 bg-slate-800/50 mx-0.5 md:mx-1" />

          <button
            onClick={togglePlay}
            className={`${localTheme.btnBase} ${localTheme.btnPrimary(isPlaying)} px-3 md:px-4`}
          >
            {isPlaying ? (
            ...
            <span className="hidden md:inline">{isPlaying ? "Pause" : "Auto"}</span>
          </button>

          <button
            onClick={prevStep}
            disabled={!canPrev}
            className={`${localTheme.btnBase} ${localTheme.btnGhost} px-3 md:px-4`}
          >
          ...
          >
            <span className="hidden md:inline">{isFinished ? "Restart" : buttonText || "Next"}</span>
            {isFinished ? (
              <RotateCcw className="w-3.5 h-3.5 flex-shrink-0" />
            ) : (
