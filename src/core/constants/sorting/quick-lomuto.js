...
private int Partition(int[] arr, int left, int right) {
  ...
  nextStep: (state) => {
    ...
    if (phase === 'READY' || phase === 'SORTED_SEGMENT') {
      ...
      return {
        ...newState, phase: 'PARTITIONING', currentCall: nextCall,
        partitionState: { i: nextCall.low, j: nextCall.low, pivot },
        activeIndices: [nextCall.high], pivotIndex: nextCall.high,
        log: { title: 'Partitioning', type: 'info', messageKey: 'START_PARTITION', params: { l: nextCall.low, r: nextCall.high, pivot } }
      };
    }
    ...
    if (phase === 'PARTITIONING') {
      ...
      if (j < high) {
        ...
        if (array[j] < pivot) {
          ...
          return {
            ...newState, array: newArray, partitionState: { ...partitionState, i: i + 1, j: j + 1 },
            activeIndices: [i, j], swapIndices: [i, j], pivotIndex: high,
            log: { title: 'Swapping', type: 'match', messageKey: 'SWAP_SMALLER', params: { val: array[j], pivot, nextI: i + 1 } }
          };
        } else {
          return {
            ...newState, partitionState: { ...partitionState, j: j + 1 },
            activeIndices: [j], pivotIndex: high,
      ...
      return {
        ...newState, array: newArray, phase: 'PIVOT_PLACED_PHASE',
        pivotPos: i, activeIndices: [i, high], swapIndices: [i, high],
        log: { title: 'Placing Pivot', type: 'shift', messageKey: 'PLACE_PIVOT', params: { pivot, r: high, pivotPos: i } }
      };
    }
    ...
    if (phase === 'PIVOT_PLACED_PHASE') {
      ...
      return {
        ...newState, phase: 'SORTED_SEGMENT', calls: newCalls,
        sortedIndices: [...sortedIndices, pivotPos], pivotIndex: null,
