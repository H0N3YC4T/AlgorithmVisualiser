...
export const bubble = createAlgorithmCard({
  ...
  getInitialState: (p, t) => {
    ...
    return {
      phase: 1, i: 0, j: 0, swapped: false, array,
  ...
  nextStep: (state) => {
    ...
    if (phase === 1) {
      ...
      if (array[j] > array[j + 1]) {
        return { 
          ...newState, phase: 2,
          log: { title: 'OUT OF ORDER', type: 'mismatch', messageKey: 'OUT_OF_ORDER', params: { j: j, valJ: array[j], jPlusOne: j + 1, valJPlusOne: array[j + 1] } } 
        };
      }
      const nextJ = j + 1;
    ...
    if (phase === 2) {
      ...
      const nextJ = j + 1;
      return { 
        ...newState, array: newArray, phase: 1, j: nextJ, swapped: true, 
        swapIndices: [j, j + 1], activeIndices: nextJ < n - 1 - i ? [nextJ, nextJ + 1] : [],
        log: { title: 'SWAPPED', type: 'shift', messageKey: 'SWAPPED', params: { j: j, jPlusOne: j + 1, valNewJPlusOne: newArray[j + 1], valNewJ: newArray[j] } } 
      };
    }
    return newState;
  }
