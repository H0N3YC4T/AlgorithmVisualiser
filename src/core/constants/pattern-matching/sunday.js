...
export const sunday = createAlgorithmCard({
  id: 'sunday',
  ...
  shiftTable = PrecomputeShifts(pattern)
  s = 0
  while s <= n - m:
    if Match(text[s...s+m-1], pattern):
      return s
    if s + m >= n: break
    s = s + shiftTable[text[s + m]]`,
...
function buildShiftTable(pattern) {
  ...
  nextStep: (state, target, pattern, preprocessing) => {
    ...
    const m = pattern.length;
    const { shiftTable } = preprocessing;

    if (phase === 1) { // Starting window
      if (currentIndex > n - m) {
        return { ...state, isFinished: true, activeIndices: [], log: { title: 'Not Found', type: 'error', message: 'Pattern not found in text.' } };
      }
      return { 
        ...state, phase: 2, compIdx: 0, activeIndices: [currentIndex],
        log: { title: 'Starting Window', type: 'info', messageKey: 'START_PHASE', params: { currentIndex, targetRange: target.substr(currentIndex, m) } } 
      };
    }

    if (phase === 2) { // Comparing
      if (target[currentIndex + compIdx] === pattern[compIdx]) {
        if (compIdx === m - 1) {
          return { 
            ...state, isFinished: true, activeIndices: [...new Array(m).keys()].map(i => currentIndex + i),
            log: { title: 'Match Found ✓', type: 'success', messageKey: 'SUCCESS_MATCH_FOUND', params: { currentIndex, m } } 
          };
        }
        return { 
          ...state, compIdx: compIdx + 1, activeIndices: [currentIndex + compIdx + 1],
          log: { title: 'Character Match', type: 'match', messageKey: 'CHAR_MATCH', params: { targetChar: target[currentIndex + compIdx], patternChar: pattern[compIdx], compIdx } } 
        };
      } else {
        return { 
          ...state, phase: 3, mismatchFound: true, activeIndices: [currentIndex + compIdx],
          log: { title: 'Mismatch', type: 'mismatch', messageKey: 'MISMATCH_DETECTED', params: { targetChar: target[currentIndex + compIdx], patternChar: pattern[compIdx], textIdx: currentIndex + compIdx } } 
        };
      }
    }

    if (phase === 3) { // Identify Lookahead
      const lookAheadIdx = currentIndex + m;
      if (lookAheadIdx >= n) {
        return { 
          ...state, isFinished: true, activeIndices: [],
          log: { title: 'Search Terminated', type: 'error', messageKey: 'SEARCH_TERMINATED', params: { lookAheadIdx } } 
        };
      }
      return { 
        ...state, phase: 4, lookAheadIdx, activeIndices: [lookAheadIdx],
        log: { title: 'Lookahead Heuristic', type: 'info', messageKey: 'IDENTIFY_LOOKAHEAD', params: { char: target[lookAheadIdx], lookAheadIdx } } 
      };
    }

    if (phase === 4) { // Execute Shift
      const lookAheadChar = target[currentIndex + m];
      const shift = shiftTable[lookAheadChar] || (m + 1);
      return { 
        ...state, phase: 1, currentIndex: currentIndex + shift, mismatchFound: false, shiftValue: shift,
        log: { title: 'Executing Shift', type: 'shift', messageKey: 'EXECUTING_SHIFT', params: { shiftValue: shift, nextPos: currentIndex + shift } } 
      };
    }
