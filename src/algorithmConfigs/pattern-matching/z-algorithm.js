...
// --- Logic Helpers (Defined outside to avoid circular reference) ---
...
export const zalgorithm = createAlgorithmPage({
  ...
  homeCard: {
    ...
    difficulty: "Hard",
    description:
      "Constructs the Z-array, where each element Z[i] is the length of the longest substring starting from i which is also a prefix.",
    complexity: {
      timeBest: "׸(n+m)",
      timeAvg: "׸(n+m)",
      timeWorst: "׸(n+m)",
  ...
  algorithmPage: {
    ...
    extendedDescription:
      'The Z-algorithm builds a "Z-array" where Z[i] is the length of the longest substring starting from concat[i] which is also a prefix of concat. It uses previously computed Z-values (Z-box) to speed up calculations.',
    legendItems: [
  ...
  nextStep: (state, target, pattern, _preprocessing) => {
    ...
    if (phase === 1) return handleDecisionPhase(newState, pattern, n);
    if (phase === 2) return handleManualComparisonPhase(newState, n);
    return newState;
  },
});

const handleDecisionPhase = (state, pattern, n) => {
  const { i, l, r, z } = state;
  if (i >= n) {
    const matches = [];
  ...
  if (i > r)
    return {
      ...
      r: i - 1,
      activeIndices: [i, 0],
      log: { title: "OUTSIDE Z-BOX", type: "info", messageKey: "OUTSIDE_Z_BOX", params: { i, r } },
    };
  const k = i - l,
    remaining = r - i + 1;
  if (z[k] < remaining) {
  ...
  return {
    ...
    log: { title: "EXTENDING Z-BOX", type: "info", messageKey: "EXTENDING_Z_BOX", params: { i, k, r } },
  };
};
