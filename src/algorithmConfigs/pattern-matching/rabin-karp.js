...
const handleInitHashPhase = (state, target, pattern, preprocessing) => {
...
const handleCompareHashesPhase = (state, pattern) => {
...
const handleVerificationPhase = (state, target, pattern) => {
...
const handleRollHashPhase = (state, target, pattern, preprocessing) => {
...
export const rabinkarp = createAlgorithmPage({
  ...
  nextStep: (state, target, pattern, preprocessing) => {
    ...
    if (phase === 0) return handleInitHashPhase(newState, target, pattern, preprocessing);
    if (phase === 1) return handleCompareHashesPhase(newState, pattern);
    if (phase === 2) return handleVerificationPhase(newState, target, pattern);
    if (phase === 3) return handleRollHashPhase(newState, target, pattern, preprocessing);
    return newState;
  },
});
