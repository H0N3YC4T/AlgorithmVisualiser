...
export const heapsort = createAlgorithmPage({
  id: "heapsort",
...
private void Heapify(int[] arr, int n, int i) {
  ...
  nextStep: (state) => {
    const { array, i, heapSize, phase, sortedIndices, siftIdx } = state;
    ...
    if (phase === 1) {
      ...
      // MaxHeapify(siftIdx || i)
      return heapifyStep(newState, heapSize, siftIdx !== null ? siftIdx : i, 1);
    }
    ...
    if (phase === 2) {
      ...
      // Extraction Step
      if (newState.subPhase === "HEAPIFYING") {
        return heapifyStep(newState, heapSize, siftIdx !== null ? siftIdx : 0, 2);
      }
      ...
      return {
        ...
        subPhase: "HEAPIFYING",
        siftIdx: 0,
        swapIndices: [0, heapSize - 1],
        activeIndices: [0, heapSize - 1],
        sortedIndices: [...sortedIndices, heapSize - 1],
        log: {
          title: "Extracting Max",
          type: "shift",
          messageKey: "EXTRACT_MAX",
          params: { valRoot: array[0], i: heapSize - 1 },
        },
      };
    }
    return newState;
...
function heapifyStep(state, n, idx, nextPhase) {
  const { array } = state;
  let largest = idx;
  let l = 2 * idx + 1,
    r = 2 * idx + 2;
  ...
  if (largest !== idx) {
    const newArray = [...array];
    [newArray[idx], newArray[largest]] = [newArray[largest], newArray[idx]];
    return {
      ...state,
      array: newArray,
      siftIdx: largest,
      activeIndices: [idx, largest],
      swapIndices: [idx, largest],
      log: {
        title: "Sifting Down",
        type: "mismatch",
        messageKey: "SIFT_DOWN",
        params: { valParent: array[idx], valChild: array[largest] },
      },
    };
  }
  ...
  if (nextPhase === 1) {
    ...
    return {
      ...
      i: nextI,
      siftIdx: null,
      log: { title: "Heapifying", type: "info", messageKey: "HEAPIFYING", params: { i: nextI } },
    };
  } else {
    return {
      ...state,
